...
export function useToolbarState(props: ToolbarProps) {
  ...
  const enterDrawMode = useCallback(
    ...
    [canvas, onSelectModeChange, playSound, applyBrush, activeColor, brushSizes],
  );
  ...
  const enterSelectMode = useCallback(() => {
    ...
    canvas.renderAll();
  }, [canvas, onSelectModeChange, playSound]);
  ...
  const selectColor = useCallback(
    ...
    [canvas, isSelectMode, onColorChange, playSound],
  );
...
export function ToolbarBottom({ state }: { state: ReturnType<typeof useToolbarState> }) {
  ...
  return (
    <div
      className="bg-white/95 backdrop-blur-sm border-t-2 border-gray-100 shrink-0"
      style={{
...
/** @deprecated Use ToolbarTop + ToolbarBottom with useToolbarState instead */
export function Toolbar(props: ToolbarProps) {
  const state = useToolbarState(props);
  return (
    <div className="flex flex-col w-full pointer-events-auto">
      <ToolbarTop state={state} />
      <ToolbarBottom state={state} />
    </div>
  );
}
